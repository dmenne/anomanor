...
cleanup_test_data = function() {
  if (getenv_r_config_active() == "test") {
    ano_pool_close()
    try(unlink(g$anomanor_data_base, recursive = TRUE), silent = TRUE)
  }
}
...
count_markers = function(record) {
  dbGetQuery(
    g$pool,
    glue_sql(
      "SELECT COUNT(*) as n from marker where record = {record}",
      .con = g$pool
    )
  )$n
}
...
get_cmp_files = function() {
  ...
  file_mtime = as.integer(unlist(lapply(record_files, function(x)
    file.info(x)$mtime)))
  record_time = dbGetQuery(g$pool, "SELECT record, file_mtime from record where valid = 1")
  ret = tibble(
