mod_admin_server = function(id, app_user) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$invite_user, {
      ...
      if (simul == "") {
        ...
        if (is.null(added)) {
          ...
          log_it(
            glue(
              "Invited user {input$new_user_email} in group ",
    ...
    save_uploaded_file = function(file) {
      ...
      if (file.exists(destination))
        shiny::showNotification(
          HTML("Could not delete file to be overwritten"),
          duration = 5,
          session = session,
          type = "error"
        )
      ...
      if (overwritten) {
        log_it(glue(
          "File {file$name} was uploaded and overwritten by {app_user}"
        ))
      } else {
      ...
      if (!file.exists(destination))
        shiny::showNotification(
          HTML("Could not copy file to destination"),
          duration = 5,
          session = session,
          type = "error"
        )
    ...
    validation_table_html = function(cv) {
      if (is.character(cv)) {
        disable("accept")
        disable("discard")
        tagList(h1("Error in anal manometry record"), h4(cv))
      } else {
    ...
    # Manage page
    observeEvent(input$drop_cache_tables, {
      ...
      dbExecute(g$pool, "DROP TABLE balloon_success")
    ...
    observeEvent(input$reset_me, {
      dbExecute(
        g$pool,
        glue_sql(
          "DELETE FROM classification where user = {app_user}",
          .con = g$pool
        )
      )
    ...
    observeEvent(input$generate, {
      generate_sample_classification(
        g$test_users,
        force = TRUE,
